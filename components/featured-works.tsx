...
export default function FeaturedWorks() {
  ...
  const nextSlide = () => {
    setCurrentIndex((prevIndex) => (prevIndex + itemsPerPage >= featuredWorks.length ? 0 : prevIndex + itemsPerPage))
  }

  const prevSlide = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex - itemsPerPage < 0 ? Math.max(0, featuredWorks.length - itemsPerPage) : prevIndex - itemsPerPage,
    )
  }

  const currentWorks = featuredWorks.slice(currentIndex, currentIndex + itemsPerPage)
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2 mt-4 md:mt-0">
            <Button variant="outline" size="icon" onClick={prevSlide} disabled={featuredWorks.length <= itemsPerPage}>
              <ChevronLeft className="h-4 w-4" />
              <span className="sr-only">Previous</span>
            </Button>
            <Button variant="outline" size="icon" onClick={nextSlide} disabled={featuredWorks.length <= itemsPerPage}>
